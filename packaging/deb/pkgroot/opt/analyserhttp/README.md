...
## Construire un paquet .deb (optionnel)
Un squelette de packaging Debian est fourni.
...
bash build.sh
```
Le fichier .deb sera généré à la racine du dossier `PythonReseau` (ex: `analyserhttp_0.1.0_all.deb`).

2) Installer le .deb
```bash
sudo apt install ./analyserhttp_0.1.0_all.deb
```

3) Lancer depuis le menu ou via
```bash
analyserhttp
```

Notes:
- Le post-install crée un environnement virtuel dans `/opt/analyserhttp/.venv` et installe les dépendances.
- Dépendances système requises: `python3`, `python3-venv`, `python3-tk`, `libpcap0.8`.
